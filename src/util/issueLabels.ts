...
export async function addLabels(context: Context<'issues'>, labelNames: string[]): Promise<void> {
	await context.octokit.rest.issues.addLabels(context.issue({ labels: labelNames }));
}

export async function removeLabel(context: Context<'issues'>, labelName: string): Promise<void> {
	if (context.payload.issue.labels?.find((label: any) => label.name === labelName)) {
		await context.octokit.rest.issues.removeLabel(context.issue({ name: labelName }));
	}
}

export async function removeLabels(context: Context<'issues'>, labelNames: string[]): Promise<void> {
